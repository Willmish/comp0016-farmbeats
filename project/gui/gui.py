...
class FarmBeatsApp:
    ...
    def labelFrameSetUp(self):
        ...
        self.label.config(background = BACKGROUND, font=("Courier", 25)) #E7F5EF
        self.label.pack()
    ...
    def optionFrameSetUp(self):
        ...
        tempImg = self.getMenuButton('assets/optionButtons/temperatureButton.png')
        tempButton = Button(self.optionFrame, image=tempImg, command = self.brightnessButtonAction, borderwidth=0) 
        tempButton.image = tempImg
        tempButton.grid(row = 0, column = 0, sticky = 'news', pady = PADDING, padx = PADDING)


        humidityImg = self.getMenuButton('assets/optionButtons/humidityButton.png')
        humidityButton = Button(self.optionFrame, image=humidityImg, command = self.brightnessButtonAction, borderwidth=0) 
        humidityButton.image = humidityImg
        humidityButton.grid(row = 0, column = 1, sticky = 'news', pady = PADDING, padx = PADDING)

        brightnessImg = self.getMenuButton('assets/optionButtons/brightnessButton.png')
        brightnessButton = Button(self.optionFrame, image=brightnessImg, command = self.brightnessButtonAction, borderwidth=0) 
        brightnessButton.image = brightnessImg
        brightnessButton.grid(row = 0, column = 2, sticky = 'news', pady = PADDING, padx = PADDING) 


        waterImg = self.getMenuButton('assets/optionButtons/waterLevelButton.png')
        waterButton = Button(self.optionFrame, image=waterImg, command = self.brightnessButtonAction, borderwidth=0) 
        waterButton.image = waterImg
        waterButton.grid(row = 1, column = 0, sticky = 'news', pady = PADDING, padx = PADDING)

        aiCameraImg = self.getMenuButton('assets/optionButtons/aiCameraButton.png')
        aiCameraButton = Button(self.optionFrame, image=aiCameraImg, command = self.brightnessButtonAction, borderwidth=0) 
        aiCameraButton.image = aiCameraImg
        aiCameraButton.grid(row = 1, column = 1, sticky = 'news', pady = PADDING, padx = PADDING)

        systemImg = self.getMenuButton('assets/optionButtons/systemVisualisationButton.png')
        sysVisualButton = Button(self.optionFrame, image=systemImg, command = self.brightnessButtonAction, borderwidth=0) 
    ...
    def profileSetUp(self, profileName):
        ...
        sensorTitle.config(background = BACKGROUND, font=("Courier", 15)) #E7F5EF
        sensorTitle.pack()

        value = Label(sensorFrame, text = profile.sensorValueDescription)
        value.config(background = BACKGROUND) #E7F5EF
        ...
        # s1 = Scale( subFrame, variable = currVal, bg=colour, sliderlength= 5, from_ = profile.bound[0], to = profile.bound[1], orient = HORIZONTAL, state = 'disabled', background = '#E7F5EF') 
        # s1.pack(side = LEFT)
        ...
        # scaleCanvas.create_text(0,30,font="Courier",
        ...
        actuatorTitle.config(background = BACKGROUND, font=("Courier", 15)) #E7F5EF
        ...
        automaticMode.config(background = BACKGROUND, font=("Courier", 15)) #E7F5EF
        ...
        actuatorVal.config(background = BACKGROUND, font=("Courier", 15)) #E7F5EF
    ...
    def graphDisplay(self,  frame , profile):
        ...
        dataFrame.plot(linewidth=0.5, kind='line', legend=True, ax=ax, color='r', fontsize=10)
