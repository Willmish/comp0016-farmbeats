...
class MoisturePidAnalyser(Analyser):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self._last_time_poured = (
            time.time() - MoisturePidAnalyser.SOAKING_IN_TIME
        )
    ...
    def soil_moisture_handler(self, args) -> None:
        ...
        if (
            ...
            if (
                time.time() - self._last_time_poured
                >= MoisturePidAnalyser.SOAKING_IN_TIME
            ):
                self._last_time_poured = time.time()
                self._cumulative_time_pump_on_since_update += (
                    MoisturePidAnalyser.POURING_TIME
                )
                sensor_data.actuator_value = 1.0
                # Sends message to the signal handler to schedule pump on
                # time and alarm signal to turn it off
                pub.sendMessage(
                    "signal_handler.pump_control",
                    signal_handler_message={
                        "pump_status": True,
                        "time_on": MoisturePidAnalyser.POURING_TIME,
                    },
                    message=sensor_data,
                )
