...
class MoisturePidAnalyser(Analyser):
    ...
    def analyser_listener(self, args, rest=None):
        ...
        clock = 5
        pub.sendMessage(f"{MAIN_PUBSUB_TOPIC}.actuator.water_pump_status", args=1.0)  # pump on
        time.sleep(output * clock)
        pub.sendMessage(f"{MAIN_PUBSUB_TOPIC}.actuator.water_pump_status", args=0)  # pump off
