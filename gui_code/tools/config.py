...
class Config:
    ...
    def set_values(self, filename: str):
        """
        set_values assigns values from .ini file that has filename as
        its file name to variables needed to be accessed by the
        ProfileInformation class to display to GUI.

        :param filename: Name of .ini file being read by ConfigParser
            object.
        :type filename: str
        """
        ...
        self.brightness_extr = self.get_list(
            self.plant_info["brightness_extr"]
        )
        self.brightness_bound = self.get_list(
            self.plant_info["brightness_bound"]
        )
        self.brightness_sensor_unit = self.plant_info["brightness_sensor_unit"]
        self.brightness_actuator_unit = self.plant_info[
            "brightness_actuator_unit"
        ]
        ...
        self.temperature_extr = self.get_list(
            self.plant_info["temperature_extr"]
        )
        self.temperature_bound = self.get_list(
            self.plant_info["temperature_bound"]
        )
        self.temperature_sensor_unit = self.plant_info[
            "temperature_sensor_unit"
        ]
        self.temperature_actuator_unit = self.plant_info[
            "temperature_actuator_unit"
        ]
        self.water_level_extr = self.get_list(
            self.plant_info["water_level_extr"]
        )
        self.water_level_bound = self.get_list(
            self.plant_info["water_level_bound"]
        )
        self.water_level_sensor_unit = self.plant_info[
            "water_level_sensor_unit"
        ]
        self.water_level_actuator_unit = self.plant_info[
            "water_level_actuator_unit"
        ]
