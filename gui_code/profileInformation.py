    ...
    def __init__(self, profileName):
        with GuiDatabaseManager() as db:
            ...
            if profileName == "Brightness":
                ...
                self.unit = "cd"
                self.sensorValueDescription = (
                    "Current value: " + str(self.sensorValue) + self.unit
                )
                ...
                self.timeList = db.get_time_and_val_list("brightness")[1]
                self.valList = db.get_time_and_val_list("brightness")[0]
                self.graphTitle = "Brightness over Time"

                self.actuatorFrameTitle = "LED light Information"
                self.actuatorValue = ACTUATION
                self.actuatorValueDescription = (
                    "Brightness set to: " + str(self.actuatorValue) + self.unit
                )
                ...
                self.actuatorValue = ACTUATION
                self.actuatorValueDescription = (
                    "Heater set to: " + str(self.actuatorValue) + self.unit
                )
